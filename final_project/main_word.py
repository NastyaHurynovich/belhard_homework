...
class PoetDataset(Dataset):
    def __init__(self, json_path, max_length=32, lowercase=True):
        ...
        self.encoded = [self.word2idx.get(word, self.word2idx['<UNK>'])
    ...
    def tokenize(self, text):
        ...
        for token in text.split():
            tokens.extend(re.findall(r"\w+|[^\w\s]", token))
        return tokens
...
class PoetTransformer(nn.Module):
    def __init__(self, vocab_size, num_authors, d_model=128, nhead=4, num_layers=4, max_seq_len=512):
        super().__init__()
    ...
    def generate(self, start_text, author_id=None, max_len=50, temperature=1.0, top_k=40):
        ...
        with torch.no_grad():
            ...
            input_ids = torch.tensor(
                [self.dataset.word2idx.get(token, self.dataset.word2idx['<UNK>'])
                 for token in tokens],
                device=device
            ).unsqueeze(0)
...
def save_checkpoint(model, optimizer, epoch, loss, checkpoint_dir="/check/checkpoints"):
